...
pub(super) fn CurrentHeritageConfig() -> Element {
    ...
    let current_heritage_config = use_memo(move || {
        ...
        let current_heritage_config = wallet_heritage_configs
            .cloned()
            .map(|wallet_heritage_configs| wallet_heritage_configs.first().cloned());
    ...
    let current_heritage_config_for_display = use_memo(move || {
        ...
        let current_heritage_config_for_display = current_heritage_config
            .cloned()
            .map(|current_heritage_config| current_heritage_config.map(Show).unwrap_or_default());
    ...
    let action_text = use_memo(move || {
        ...
        let action_text = if let Some(Some(_)) = current_heritage_config() {
            "Update"
        } else {
            "Create"
        };
    ...
    rsx! {
        div { class: "container mx-auto p-4 w-fit",
            ...
            Modal { is_open: new_heritage_config_modal, persistent: true,
                div { class: "max-w-5xl h-[calc(100vh-4rem)]",
